# Cursor Prompt: Phase 2D Implementation (Pre-Dropcontact Enhancements)

## Build the following components EXACTLY as specified:

### 1) ICP Response Processor (replace code in node `8166bd91-62cc-460b-883e-f5c64e943ab1`)
```javascript
...
### 2) Anomaly Detection (new Code node after ICP Response Processor)
```javascript
...
### 3) Junk Detection Filter (new Code node after Double Failure Router TRUE)
```javascript
...
### 4) Salvage Pre-Score Calculator (new Code node after Junk Filter)
```javascript
...
## EVIDENCE REQUIRED
- Show reduction in double-fails (target <25%)
- Show score variance reduced to ±5 on repeat runs
- Show auto-archive for `2D-JUNK-1` test
- Show salvage route for `2D-SALVAGE-1` test
- Export workflow JSON after updates

## TEST PAYLOADS
Use file: `context/phase-2d/tests/test-payloads-2d.json`
