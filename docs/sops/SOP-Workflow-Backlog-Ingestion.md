...
- **If Leads are Incorrectly Routed**: The logic for this is entirely within the `Normalize` node. Review the phone validators and the default state rules (phone-only gate) in that node's code.
